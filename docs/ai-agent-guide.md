...
| `TypeError: The dependencies object passed to a factory is read-only`   | A factory assigned to, deleted from, or defined a property on its `deps` argument                                               | Return the value instead; the container is not a scratch space                                            |

...
`add`, `update`, and `merge` all mutate `this` and return it re-cast — `merge` writes into the state's `resolvers` map and returns `this`. All three check before they write: `merge` validates every incoming name of every container in a first pass — reserved, foreign own property, non-function resolver, every resolved value backed by a resolver (a container-shaped JavaScript input can carry a phantom; a real container cannot), that the receiver is still extensible when the name needs a new getter, and that it is not frozen when the name is a replacement (a consumer can `Object.freeze` the container itself; the deps proxy guards only the factory's view) — and writes in a second, so a refused name leaves `this` untouched instead of half-merged with an eviction that cannot be undone. The key arrays are read once and reused, so it is still linear; `mergeAtomicity.test.ts` pins it. `clone()` and the static `DIContainer.compose()` are the only ways to get a genuinely separate instance; `compose` builds a fresh container and merges each input into it, leaving the inputs untouched.

`clone()` builds a plain `new DIContainer()` and hands it, with the source's maps, to `seedResolvers` — a `protected static`, so a consumer subclass constructor can seed too. It used to go through `ClonedDiContainer`, a hidden subclass whose only job was a constructor that could call the then-instance-level seeding method; a static needs no subclass, and the subclass could not live in its own file anyway (`extends DIContainer` at module top level meets the import cycle in the temporal dead zone). The clone is therefore a `DIContainer`, not an instance of the caller's class. `seedResolvers` throws if the container already has resolvers, and — because a consumer subclass can hand it anything — runs the same validation pass `merge` runs before writing (reserved names, foreign own properties, non-function resolvers) plus one rule of its own: every resolved value must have a resolver, or `has()` would deny a name `get()` returns.

**The state object's shape is a cross-version contract.** `INTERNAL_STATE` is a `Symbol.for` key so that two installed copies of rsdi can compose each other's containers — and those copies can be different versions. `merge` therefore reads a foreign container's state through `ForeignContainerState` in `internalState.ts`, a `Pick` of exactly `resolvers` and `resolvedDependencies`, so reaching for a third field is a compile error; `isContainer` checks those same two and nothing else. Adding fields to `InternalState` (as `registrations` was) is safe, because nothing reads them from the other side; renaming or removing either map breaks composition with every other version, with a message that names neither. `mergeAtomicity.test.ts` pins that a state with unknown extra fields and none of the later ones is accepted.

**Locking the container keeps its platform meaning.** `preventExtensions` and `seal` forbid new properties, so on a container they forbid new names — `add` and a merge with a new name — and nothing else. `freeze` also forbids writing existing properties, so it forbids replacing a resolver: `update` and a replacing `merge` throw a `TypeError` naming the dependency. Resolution is never refused — it writes into the cache inside the state object, which no lock reaches — and a `clone()` is a fresh, unlocked container. The freeze test is `isFrozenContainer` in helpers.ts, one descriptor read on the state symbol (only `freeze` turns `writable` off); `Object.isFrozen` walks every own property and would make each `update` linear in the number of dependencies. Before this, `update` succeeded on a frozen container, so a freeze meant to protect a shared fixture or a bootstrapped graph protected nothing.
...
**`get` caches only if nothing was registered while the factory ran.** `update` (and `merge`) evict the cache and install the new resolver in one step; when that happened _while the name's own factory was running_ — a factory reaching the container through a closure — `get` then cached the old factory's result as it returned, under the new resolver, and every later request served the stale value. The state object carries a `registrations` counter that every resolver write bumps; `get` reads it before the factory and caches only if it is unchanged after. A counter rather than resolver identity, because `update(name, sameFactory)` and a `merge` that carries the same function object are replacements too — the first asks for a fresh instance, the second brings its own resolved value — and identity cannot see either. The value is still returned to the caller that asked, since that is what ran; the next request runs whatever is registered then. A registration of an unrelated name mid-flight costs that name one re-run, which is the conservative side. `updateDuringResolution.test.ts` pins all of it.
...
**A factory's own error propagates untouched — on purpose.** When a factory throws, `get` releases the in-flight name and rethrows exactly what it caught, so `connection refused` from deep inside `container.userService` does not say which dependency's factory failed. Two ways to add that were considered and rejected: wrapping the error in a container class changes the type a consumer catches (`error instanceof ConnectionError` stops being true), and attaching the path via `error.cause` or a custom property means mutating an error the container does not own, which may be frozen, shared, or already carrying a `cause`. The stack trace carries the factory frame, and the container's own errors (`DependencyIsMissingError`, `CircularDependencyError`, the read-only `TypeError`) already name the path where the container is the one at fault. Don't add wrapping; if the need ever proves real, the answer is an opt-in hook, not a change to what `get` throws.
